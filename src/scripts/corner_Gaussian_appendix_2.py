...
samples_Gaussian = np.load(paths.data/"samples_Gaussian_without_GW170818_GW200129.npz")["chains"]
...
axs[0,0].axvline(mu_median, color=sns.color_palette()[3])
...
axs[1,1].axvline(sigma_median, color=sns.color_palette()[3])
...
axs[1,0].axvline(mu_median, color=sns.color_palette()[3])
axs[1,0].axhline(sigma_median, color=sns.color_palette()[3])
kde_contour.kdeplot_2d_clevels(xs=df['mu'], ys=df['sigma'], ylow=0, ax=axs[1,0],
...
fig.savefig(fname=paths.figures/"corner_Gaussian_appendix_2.pdf", bbox_inches="tight", dpi=300)