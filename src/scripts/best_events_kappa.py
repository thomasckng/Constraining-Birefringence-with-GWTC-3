...
with open(paths.output/"best_events_kappa.txt", "w") as f:
    f.write(r"\begin{tabular}{lrr}Event & $\kappa$ & $\sigma_i$\\ \hline ")
    for i, event in enumerate(result_dict):
        f.write(rf"{result_dict[event]['event']} & ${result_dict[event]['kappa_median']:.3f}^{{+{result_dict[event]['kappe_95']:.3f}}}_{{-{result_dict[event]['kappe_5']:.3f}}}$ & ${result_dict[event]['kappa_std']:.3f}$")
        if i != 9:
            f.write(r"\\")
        else:
            break
