...
with open(paths.output/"mu_median.txt", "w") as f:
    f.write(rf"$\mu = {mu_median:.3f}^{{+{(np.percentile(a=df['mu'],q=95)-mu_median):.3f}}}_{{{(np.percentile(a=df['mu'],q=5)-mu_median):.3f}}}$")

with open(paths.output/"sigma_median.txt", "w") as f:
    f.write(rf"$\sigma < {np.percentile(a=df['sigma'],q=90):.3f}$")
