...
\citet{Okounkova_2022} gave a constraint on frequency-independent \ac{GW} amplitude birefringence by performing statistical analysis on GWTC-2.
We take $f=100\, \mathrm{Hz}$ as the reference frequency to make the comparison.
They were able to constrain $\tilde{\kappa}$ at $d_C=d_C(z=0.3)$ to be $|\tilde{\kappa}| \lesssim 0.74$ at 1 $\sigma$, where
\begin{equation}
    \tilde{\kappa}d_C = \left[{\kappa}\frac{d_C}{1\, \mathrm{Gpc}}\frac{f}{100\, \mathrm{Hz}}\right]_{f=100\, \mathrm{Hz}}\,.
\end{equation}
Note that $\tilde{\kappa}$ is in units of $1/\mathrm{Gpc}$, while $\kappa$ is dimensionless.
Equivalently, their constraint is $|\kappa| \lesssim 0.74$ at 1 $\sigma$.
We obtained a tighter constraint on $|\kappa|$ with $|\kappa| <$ \variable{output/restricted_absolute_kappa_68.txt}at $68.27\%$ upper limit.
...
They obtained a constraint equivalent to $|\kappa| <$ \variable{output/kappa_Wang.txt}, given by the 90\% upper limit.
...
\begin{figure}
    ...
    \caption{
    Impact of birefringence on mass ratio and precession inference. Posterior on mass ratio $q \equiv m_2/m_1$ (left) and precessing spin $\chi_p$ (right) for GW170104 (top) and GW191105 (bottom), as obtained from both the birefringent analysis (orange) and the regular \ac{GR} analysis (blue).
    }
...
The events GW170818 (Sec.~\ref{sec:GW170818}) and GW190521 (Sec.~\ref{sec:GW190521}) appear to fall in these two categories respectively.
...
Since the mass ratio and spins can affect the phasing of a \ac{GW} inspiral at similar order \cite{Blanchet:2013haa}, we might expect some degree of interaction between $q$ and $\kappa$.
This seems to indeed be the case for some events, like GW170104 or GW191105, as we demonstrate in Fig.~\ref{fig:mass_ratio}.
In the former, introducing birefringence increases the preference for asymmetric masses ($q < 1$), while it decreases the inferred level of precession; in the latter, birefringence increases the preference for symmetric masses ($q = 1$) and does not affect the inference on precession.
...
Equivalently, the constraint on $\kappa$ is $|\kappa| <$ \variable{output/kappa_Wang.txt}.
