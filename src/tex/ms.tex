...
Following \cite{Alexander:2009tp}, the effect of birefringence can be modeled as a frequency-dependent amplification or dampening;
\added{similar derivations can be found in \cite{Zhao:2019xmm, Ezquiaga:2021ler, Zhu:2023wci}.}
...
\begin{equation}
    \left|\kappa\right| \left(d_c/1\, \mathrm{Gpc}\right) \left(f/100\, \mathrm{Hz}\right) < 1\, \mathrm{Gpc}^{-1}\,.
...
This quantity can be understood as the relative height of the probability density function at a given value, and does not assume any symmetric interval or modality.
...
\begin{figure}
    ...
    \caption{
        ...
        Note that the condition in Eq.~\eqref{eq:small_exponent} is not violated as the distance posterior shown here is the posterior on the luminosity distance $d_L$, not on the comoving distance $d_C$.
    }
...
\begin{equation} \label{eq:kappa-to-Mpv}
...
Note that our constraints from each event are stronger compared to the constraint from \citet{Wang_2021}, details of which are given in Sec.~\ref{sec:comparison_Zhu}.
...
\citet{Zhu:2023wci} reported a constraint of $M_{\rm PV} > 4.1 \times 10^{-22}\, {\rm GeV}$ at 90\% credibility, which translates into \variable{output/kappa_Zhu.txt} per Eq.~\eqref{eq:kappa-to-Mpv} above.

This constraint is weaker than ours in spite of the similar number of events considered, a discrepancy that might be explained by a number of factors.
First, there are difference in priors, since \citet{Zhu:2023wci} assume a distribution uniform in $z/M_{\rm PV}$ rather than $\kappa$ or $M_{\rm PV}$ itself.
Second, as reported in \citep{Zhu:2023wci}, their combined result was computed by multiplying $M_{\rm PV}^{-1}$ posteriors from individual events; however, this is not the correct way of combining $M_{\rm PV}$ measurements from independent observations: instead of multiplying posteriors, one should multiply the \emph{likelihoods}Z to avoid double counting the prior every time a new event is added to the combined constraint.
Finally, \citet{Zhu:2023wci} report \emph{ad hoc} exclusion of certain events from the combined constraint, without explicitly providing a list of events considered.
All these factors combined preclude a clear interpretation of the results in \citep{Zhu:2023wci}.
In \ref{fig:posterior_MPV}, we show the posterior on $M_{\rm PV}^{-1}$ for each event in our analysis.
Compared to Fig.~1 in \citet{Zhu:2023wci}, we can see that the posteriors on some events are different \added[id=WF]{in both scale and shape} between our analysis and theirs.
...
This shows that the difference in the combined constraint may come from the difference in the individual event analyses.
...
\begin{figure}
    \script{posterior_MPV.py}
    \includegraphics[width=\columnwidth]{figures/posterior_MPV.pdf}
    \caption{
        Individual-event $M_\mathrm{PV}^{-1}$ posteriors from our analysis, converted from the $\kappa$ posteriors in Fig.~\ref{fig:violin_kappa}.
        Chosen events are those labeled in Fig.~1 of \citet{Zhu:2023wci}.
    }
...
Encouraged by Chern-Simons gravity, this study only focused on amplitude birefringence.
Other parity-violating gravity theories also predict velocity birefringence, which dominates over amplitude birefringence in terms of amplitude and phase corrections to the waveform when both birefringence are present \cite{Zhao:2019xmm}.
