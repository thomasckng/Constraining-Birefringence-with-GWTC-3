...
\begin{abstract}
    ...
    From 71 binary-black-hole signals, we obtain the most precise constraints on gravitational-wave amplitude birefringence yet, measuring a birefringent opacity of $\kappa =$ \variable{output/restricted_kappa_median.txt} with 90\% credibility, equivalent to an energy scale of $M_{\rm PV} \lesssim XX\, {\rm GeV}$ \mi{Thomas please provide the equivalent $M_{\rm PV}$ constraint from $|\kappa|$)}.
\end{abstract}
...
On the other hand, our constraint from Sec.~\ref{sec:results:gwtc} is $|\kappa| <$ \variable{output/restricted_absolute_kappa_68.txt} at 68\% credibility.
...
\end{equation}
\citet{Wang_2021} quote a constraint of $M_{\rm PV} > 10^{-22}\, {\rm GeV}$ at 90\% credibility, which translates into $|\kappa| \lesssim$ \variable{output/kappa_Wang.txt}.
