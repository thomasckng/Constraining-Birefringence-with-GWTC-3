...
This frequency dependence provides stronger enhancement or suppression from the birefringence to high frequency components of the GW, and visa versa. \kw{Redundant.}
Also, \kw{You don't need an also here since you are still talking about the same thing} the frequency dependence can break the degeneracy between $\kappa$ and $\iota$, as this would make the effect of the birefringence affect the amplitude ratio differently compared to $\iota$.
...
GR test.}. These control tests result can be found in the data release.
\kw{Link}
...
$\mu$ and $\sigma$ are \variable{output/Gaussian_mean.txt}, respectively.
