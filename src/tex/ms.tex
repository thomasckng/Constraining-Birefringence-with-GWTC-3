...
The two events that show the largest tension with $\kappa = 0$ is GW170818, for which $|\mu_i / \sigma_i| = \variable{output/GW170818_constraint.txt}$, and GW200129\_065458 (henceforth GW200129), for which $|\mu_i / \sigma_i| = \dots$.
\mi{THOMAS: please add the number for GW200129. Also, we don't need that many significant figures, one decimal will do.}
...
The hierarchical measurement leads to an expectation that $\kappa_i = $\variable{output/restricted_kappa_median.txt}, whereas the shared-$\kappa$ measurement implies $\kappa = $\variable{output/generic_kappa_median.txt}, both symmetric 90\%-credible intervals around the median.
...
\begin{figure}
    ...
    \caption{
        ...
        \mi{THOMAS: please add a second set of curves \emph{without} GW200129; use the same colors you have now but plot them with a dashed trace}
    }
...
When considered in isolation, the first of these is the event with the most significant support for $\kappa \neq 0$; the second shows sign of potential noise anomalies; and the third is representative of a class of events with strongly bimodal $\kappa$ posteriors.

\subsubsection{GW170818}
GW170818 gave the largest tension with $\kappa=0$, when judged by $\mu_i/\sigma_i$ in Fig.~\ref{fig:violin_kappa}.
Figure \ref{fig:corner_GW170818} shows the birefringence effect allows a region of $\kappa>0$ for
 larger distance and a smaller inclination angles than would be allowed in the absence of birefringence.
We can make sense of this result because a smaller inclination angle will make the amplitude ratio between the two circular polarizations smaller.
...
\mi{THOMAS: did you have a comment to make about this case being similar to GW150914? Also, were there any other notable differences between the birefringence posterior and the LVK one? e.g., in other parameters like the masses or spins?}
...
However, data for this event were affected by a non-Gaussian noise disturbance (glitch) in the Virgo instrument, which was subtracted in the publicly-available data used for parameter estimation \cite{Davis:2022ird}.
...
\subsubsection{Result on GW190521}
