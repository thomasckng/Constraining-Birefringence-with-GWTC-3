...
Compared to Ref.~\citep{Yamada_2020,Wang_2021,Okounkova_2022}, we perform \ac{PE} on more events, including events new to GWTC-3 \citep{GWTC-3}, and use a phenomenology-oriented parametrization.
...
Similar derivations can be found in \cite{Zhao:2019xmm, Ezquiaga:2021ler, Zhu:2023wci}.
...
\begin{figure}
    ...
    \caption{
        ...
        Solid (dashed) traces indicate the analysis including (excluding) GW200129.
...
Their model is similar to the one used in \citet{Wang_2021}, which allows us to compare our results, as we did in Sec.~\ref{sec:comparison_Wang}.
...
This result constrain the birefringence effect much weaker than our result.
...
However, we could not compare the analysis in \citet{Zhu:2023wci} with ours in detail, as the analysis is not publicly available, and the list of events included is not provided in the paper.
