...
Our primary constraint on birefringence is given by the GWTC-3 combined constraint on the birefringent opacity, $\kappa =$ \variable{output/restricted_kappa_median.txt} with 90\% credibility, obtained in Sec.~\ref{sec:results:gwtc} (blue band in Fig.~\ref{fig:violin_kappa}).
This can be compared to previous measurements in the literature.
...
Taking a reference frequency of $f=100\, \mathrm{Hz}$, which is representative of LIGO-Virgo events, we may compare that constraint to ours by noting that
\begin{equation}
    \tilde{\kappa}d_C = \left[{\kappa}\frac{d_C}{1\, \mathrm{Gpc}}\frac{f}{100\, \mathrm{Hz}}\right]_{f=100\, \mathrm{Hz}}\,.
\end{equation}
per Eq.~\eqref{eq:waveform_modification}, and hence $\tilde{\kappa} = \kappa / {\rm Gpc}$ at $f=100\, {\rm Hz}$.
In terms of this fiducial quantity, our constraint becomes $|\tilde{\kappa}| <$ \variable{output/restricted_absolute_kappa_68.txt} ${\rm Gpc}^{-1}$ at 68\% credibility.
...
Since the mass ratio and can affect the phasing of a \ac{GW} inspiral at similar order as the spins \cite{Blanchet:2013haa}, we might expect some degree of interaction between $q$ and $\kappa$.
