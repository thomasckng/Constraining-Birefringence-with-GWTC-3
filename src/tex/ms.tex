...
Their model is the same as used in \citet{Wang_2021}, which allows us to compare our results, as we did in Sec.~\ref{sec:comparison_Wang}.
\citet{Zhu:2023wci} reported a constraint of $M_{\rm PV} > 4.1 \times 10^{-22}\, {\rm GeV}$ at 90\% credibility, which translates into \variable{output/kappa_Zhu.txt}.
This result constrains the birefringence effect much weaker than our result.
This is an unexpected result, as the analysis in \citet{Zhu:2023wci} is similar to ours\footnote{Note that the prior on the birefringence parameter and luminosity distance are different between the analysis by \citet{Zhu:2023wci}and ours, but it is unlikely to be the cause of the difference in the constraint.} and they included more events than we did.
