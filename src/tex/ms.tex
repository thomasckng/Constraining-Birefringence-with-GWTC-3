...
\added{Following \cite{Alexander:2009tp}, the effect of birefringence can be modeled as a frequency-dependent amplification or dampening; similar derivations can be found in \cite{Zhao:2019xmm, Ezquiaga:2021ler, Zhu:2023wci}.}
...
\begin{equation}
    \left|\kappa\right| \left(d_c\added{/1\, \mathrm{Gpc}}\right) \left(f/100\, \mathrm{Hz}\right) < 1\, \added{\mathrm{Gpc}^{-1}}\,.
...
\added{This quantity can be understood as the relative height of the probability density function at a given value, and does not assume any symmetric interval or modality.}
...
\begin{figure}
    ...
    \caption{
        ...
        \added{Note that the condition in Eq.~\eqref{eq:small_exponent} is not violated as the distance posterior shown here is the posterior on the luminosity distance $d_L$, not on the comoving distance $d_C$.}
    }
...
\added{Note that our constraints from each event are stronger compared to the constraint from \citet{Wang_2021}, details of which are given in Sec.~\ref{sec:comparison_Zhu}.}
...
Compared to Fig.~1 in \citet{Zhu:2023wci}, we can see that the posteriors on some events are different in both scale and shape between our analysis and theirs.
