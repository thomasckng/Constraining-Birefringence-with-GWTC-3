...
Our primary constraint on birefringence is given by the combined constraint on the birefringent opacity from GWTC-3, $\kappa =$ \variable{output/restricted_kappa_median.txt} with 90\% credibility, obtained in Sec.~\ref{sec:results:gwtc} (blue band in Fig.~\ref{fig:violin_kappa}).
...
\begin{equation}
    \left[ \tilde{\kappa} = \frac{\kappa}{1\, \mathrm{Gpc}} \right]_{\raisebox{1ex}{$\begin{subarray}{l} z=0.3 \\ f=100\,{\rm Hz} \end{subarray}$}}
\end{equation}
per Eq.~\eqref{eq:waveform_modification}.
In terms of this fiducial quantity, our constraint from Sec.~\ref{sec:results:gwtc} can be expressed as $|\tilde{\kappa}| <$ \variable{output/restricted_absolute_kappa_68.txt} ${\rm Gpc}^{-1}$ at 68\% credibility.
This is a factor of ${\sim}25\times$ \mi{Thomas, can you make this number be dynamically generated? it should be the ratio of Masha's constraint to ours, rounded to the closest integer} more stringent than Ref.~\cite{Okounkova_2022}, besides arising from a less simplified model.
...
Comparing to our Eq.~\eqref{eq:waveform_modification} and approximating $z \approx d_C H_0/c$ with the Hubble constant $H_0$, this means
\begin{equation}
    |\kappa| = \frac{\pi H_0}{c}\left(1\,\mathrm{Gpc}\right)\left(100\,\mathrm{Hz}\right)M_{\rm PV}^{-1}\,.
\end{equation}
\citet{Wang_2021} quote a constraint of $M_{\rm PV} > 10^{-22}\, {\rm GeV}$ at 90\% credibility, which translates into $|\kappa| <$ \variable{output/kappa_Wang.txt}.
