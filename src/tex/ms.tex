...
This posterior supports lack of birefringence ($\mu = \sigma = 0$) within 90\% credibility; from the marginals, we constrain $\mu =$ \variable{output/mu_median.txt} and $\sigma =$ \variable{output/sigma_median.txt}, for 90\%-credible symmetric intervals around the median \mi{THOMAS: for $\sigma$ it would make more sense to report the one-sided 90\%-credible upper limit here, i.e.i, the 90th percentile, as $\sigma < XX$}.
