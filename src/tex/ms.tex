...
\begin{abstract}
    ...
    Here we look for evidence of amplitude birefringence in the \replaced{third gravitational-wave transient}{latest Laser Interferometer Gravitational Wave Observatory-Virgo} catalog (GWTC-3) through the use of birefringent templates inspired by dynamical Chern-Simons gravity.
...
Finally, we conclude in Sec.~\replaced{\ref{sec:Conclusion}}{\ref{sec:Discussion}} with a summary.
\added{Additional details on the results for some notable events are presented in \ref{sec:Appendix}.}
...
\begin{figure}
    ...
    \caption{
        ...
        However, if spacetime were birefringent following Eq.~\protect\eqref{eq:waveform_modification}, then the waveform observed on Earth would contain different fractions of the two circular modes, with higher frequencies affected more strongly (solid, top). 
...
The result of \citet{Okounkova_2022} is also phrased in terms of a canonical Chern-Simons length scale,\footnote{This results from setting the so-called ``canonical'' Chern-Simons embedding into an Friedmann-Robertson-Walker cosmology, where the Chern-Simons field evolves as $\theta \propto t$ \cite{Alexander:2009tp,Jackiw:2003pm,Yunes2009}.}
\begin{equation}
