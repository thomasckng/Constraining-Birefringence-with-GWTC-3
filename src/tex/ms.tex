...
Eq.~\eqref{eq:waveform_modification} can be derived as the first order effect in an expansion away from \ac{GR} under multiple frameworks.
...
Fig.~\ref{fig:violin_kappa} makes it clear that not all \acp{BBH} in GWTC-3 are equally informative about birefringence.
When considered individually, the events that best constrain $\kappa$ are listed in Table~\ref{tab:best_events_kappa}.
\begin{table}
    \caption{Events that best constrain $\kappa$, where $\tilde{\kappa}$ is the posterior median of $\kappa$.} 
    \begin{ruledtabular}
...
\begin{table}
    \caption{Events that show bimodality in the $\kappa$ posterior, where the total mass of the binary is given in solar masses.}
    \begin{ruledtabular}
...
Fig.~\ref{fig:violin_kappa} shows some variance in the distribution of $\kappa$ posteriors, including some apparent outliers like GW170818.
This is not unexpected: assuming independent Gaussian noise, we might expect up to ${\sim}3$ events in a sample of 71 events to deviate from the true value of $\kappa$ by ${\sim}2\sigma$.
\mi{WIP}

% Corner plot of the mean and standard deviation of kappa
We then show the result of Bayesian hierarchical modeling in Fig.~\ref{fig:corner_Gaussian}.
...
\section{Reweighed kappa distributions}
\label{sec:reweighed_kappa}

\begin{figure}
    \script{reweighed_kappa.py}
    \includegraphics[width=\columnwidth]{figures/reweighed_kappa.pdf}
    \caption{Reweighed $\kappa$ distributions.}
    \label{fig:reweighed_kappa}
\end{figure}
