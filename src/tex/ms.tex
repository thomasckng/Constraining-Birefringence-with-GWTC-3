...
% Motivation
Gravitational waves (GWs) have been used to test Einstein's theory of general relativity (GR) since the first detection in 2015.
...
% cite ligo, virgo
\citet{Yamada_2020} and \citet{Wang_2021} both performed parameter estimation (PE) on the events in GWTC-1, the first GW transient catalog, using birefringence templates.
% cite GWTC-1
\citet{Okounkova_2022} considered the distribution of observed inclinations of the GW events in GWTC-2, the second GW transient catalog, to look for signs of birefringence.
% cite GWTC-2

% What's new?
In this study, we used a frequency-dependent birefringence model to constrain the strength of GW amplitude birefringence.
...
% remove GW200129 from the footnote
...
% For a nonprecessing binary, the amplitude ratio of the left-handed and right-handed polarizations only depends on the inclination. dominated by quadrupolar mode radiation. 2-2 mode
...
% birfringence can alter the ratio of the polarization of the waveform
Thus, the birefringence effect will affect the PE result on $\iota$.
% will -> could/ without freq the effect will affect
%base on masha
...
We then perform PE on intrinsic and extrinsic parameters including $\kappa$.
...
% cite the zenodo data release in bib
...
% under XXX, kappa should share the same value
% to understand more, the true value of kappa is drawn from a distribution
% the distribution should be consistent with a delta function
% if it is not a delta function, there should be more study (model not fully correct, new physics, or systematic error)

% we expect the kappa to be a single value for all events at the beginning
...
% instead of assuming all event share one value, we assume the kappa is a true value from a distribution.
...
% we expect the delta should be 
In this study, we assume the underlying distribution is a Gaussian distribution with mean $\mu$ and standard deviation $\sigma$.
% Gaussian max entropy, whatever the true distribution is, the mean and the standard deviation will be true.
...
\end{figure*}
% order time, color median/std
% symmetrical credible interval 5 to 95 percentile (np.percentile)
...
% include both Hierarchical and stacking
