...
\begin{table}
    \caption{Events with bimodality in the $\kappa$ posterior, the \ac{GR} measurement of their detector-frame total mass ($M$), their $\chi_p$ and $\chi_{\rm eff}$ measured without birefringence, and the credible level of $\kappa = 0$ (CL) with birefringence.}
    \begin{ruledtabular}
...
They were able to constrain $\kappa$ to be $|\kappa| \lesssim 0.74$ at 1 standard deviation.
We obtained a tighter constraint on $|\kappa|$ with $|\kappa| <$ \variable{output/restricted_kappa_std.txt}at 1 standard deviation.
...
They obtained a constraint equivalent to $|\kappa| \lesssim 0.30$, given by the 90\% upper limit.
We obtained a tighter 90\% upper limit of $|\kappa| <$ \variable{output/restricted_absolute_kappa_90.txt}.

Our result is an order of magnitude improvement in constraining $\kappa$.
The main reason is that we included more events from GWTC-3 compared to \citet{Okounkova_2022} and \citet{Wang_2021}.
...
\begin{figure}
    ...
    \caption{
    Impact of birefringence on mass ratio inference. Posterior on $q \equiv m_2/m_1$ for GW170104 (top) and GW191105 (bottom), as obtained from both the birefringent analysis (orange) and the regular \ac{GR} analysis (blue).
    }
    \label{fig:mass_ratio}
\end{figure}


In examining our results in Sec.~\ref{sec:results:notable}, we identified a number of interactions between birefringence and the inference of source properties like the component spin parameters---in addition to partial degeneracies with inclination and distance already discussed in the literature.
We found that the frequency-dependent modification to the waveform induced by birefringence can effectively mimic spin effects, including precession, especially for heavier systems which are observed over short durations.

As a consequence, a system that is inferred to be highly spinning in \ac{GR} may instead be inferred to be low-spinning but highly birefringent.
These degeneracies can interact with the distribution of prior volume to yield a preference for nonzero $\kappa$ in some events.
The resulting posterior can be bimodal or unimodal depending on the specific phase and polarization states observed at the detector.

Conversely, we expect that a preference for birefringence can also appear for signals that do not show significant spin effects as long as the spin or other source parameters can be tuned to counteract the frequency-dependent dephasing induced by a nonzero $\kappa$.
...
Since mass ratio and spins can have similar effects, we expect some interactions could take place between $q$ and $\kappa$.
This seems to indeed be the case for some events, like GW170104 or GW191105, as we demonstrate in Fig.~\ref{fig:mass_ratio}.
This degeneracy is likely more limited since mass ratio alone cannot produce significant amplitude modulations.
...
We found no evidence of amplitude birefringence in the GWTC-3 data, and constrained $\kappa =$ \variable{output/restricted_kappa_median.txt}with 90\% credibility when treating $\kappa$ as a global quantity shared by all GWTC-3 events (Sec.~\ref{sec:results:gwtc}).
...
In particular, we identified the relevance of spins their (partial) degeneracy with $\kappa$, in addition to expected correlations with source inclination and distance.
...
% \subsection{Extended corner plot for GW191105}
% \label{sec:corner_GW191105_appendix}
% 
% \begin{figure*}[h]
%     \script{corner_GW191105_appendix.py}
%     \includegraphics[width=\textwidth]{figures/corner_GW191105_appendix.pdf}
%     \caption{
%         Extended corner plot for GW191105: a supplement to Fig.~\ref{fig:corner_GW191105} discussed in Appendix~\ref{sec:corner_GW191105_appendix}.
%     }
%     \label{fig:corner_GW191105_appendix}
% \end{figure*}

% \section{Extended corner plot for GW200129}
% \label{sec:corner_GW200129_appendix}
% 
% \begin{figure*}[h]
%     \script{corner_GW200129_appendix.py}
%     \includegraphics[width=\textwidth]{figures/corner_GW200129_appendix.pdf}
%     \caption{
%         Extended corner plot for GW200129: a supplement to Fig.~\ref{fig:corner_GW200129} discussed in Appendix~\ref{sec:corner_GW200129_appendix}.
%     }
%     \label{fig:corner_GW200129_appendix}
% \end{figure*}
...
\end{equation}
Using $H_0d_C=cz$, where $H_0$ is the Hubble constant and $c$ is the speed of light, and adding back the factor of Planck constant $h$, we can rewrite the equation as
\begin{equation}
    |\kappa|=\frac{h\pi H_0}{c}\left(1\,\mathrm{Gpc}\right)\left(100\,\mathrm{Hz}\right)M_{PV}^{-1}\,.
\end{equation}
They gave the constraint on $M_{PV}$ as $M_{PV}>10^{-22}\,\mathrm{GeV}$.
This constraint only considered the amplitude birefringence modification.
Equivalently, the constraint on $\kappa$ is $|\kappa| \lesssim 0.30$.
