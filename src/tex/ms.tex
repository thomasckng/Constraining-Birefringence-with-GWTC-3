...
\mi{caveats?}
Under this assumption, the posterior on $\kappa$ inferred collectively from all events with a uniform prior is simply the product of the individual posteriors,%
\footnote{Had the $\kappa$ prior been nonuniform, then the individual \emph{likelihoods} would need to be multiplied, as opposed to the posteriors.}
\begin{equation}
    p(\kappa \mid \{d_i\})\propto \prod_{i}p(\kappa \mid d_i)\,,
    \label{eq:restricted_posterior}
\end{equation}
where $d_i$ is the strain data for the $i$\textsuperscript{th} event.
We use this expression to obtain the primary constraint presented in this work.

\subsubsection{Nonshared birefringence parameters}

% Bayesian Hierarchical Modeling
In order to further characterize our set of measurements, and identify potential outliers, we carry out an additional collective analysis that allows $\kappa$ to take different values for each events.
To do this, we make use of hierarchical Bayesian inference to model the distribuion of $\kappa$'s consistent with the observed data.
